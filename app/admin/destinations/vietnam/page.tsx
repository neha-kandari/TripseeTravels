...
const VietnamAdminPage = () => {
  ...
  const fetchPackages = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.log('Fetched Vietnam packages from MongoDB API:', apiPackages.length);
        ...
        console.log(`VIETNAM PACKAGE FILTER: ${apiPackages.length} total packages, ${validPackages.length} valid packages`);
        
        const transformedPackages = validPackages.map((pkg: any) => {
          console.log('VIETNAM PACKAGE TRANSFORM: Raw package:', pkg);
          console.log('VIETNAM PACKAGE TRANSFORM: Package _id:', pkg._id);
          console.log('VIETNAM PACKAGE TRANSFORM: Package id:', pkg.id);
  ...
  const handleDelete = async (id: string) => {
    console.log('DELETE VIETNAM PACKAGE: Delete button clicked for ID:', id);
    console.log('DELETE VIETNAM PACKAGE: ID type:', typeof id);
    console.log('DELETE VIETNAM PACKAGE: ID length:', id?.length);
    ...
    if (!window.confirm('Are you sure you want to delete this package?')) {
      console.log('DELETE VIETNAM PACKAGE: User cancelled deletion');
      return;
    }

    try {
      console.log('DELETE VIETNAM PACKAGE: User confirmed deletion');
      console.log('DELETE VIETNAM PACKAGE: Calling API with URL:', `/api/admin/packages/${id}`);
      
      const response = await fetch(`/api/admin/packages/${id}`, {
        method: 'DELETE',
      });
      
      console.log('DELETE VIETNAM PACKAGE: Response status:', response.status);
      console.log('DELETE VIETNAM PACKAGE: Response ok:', response.ok);
      
      if (response.ok) {
        console.log('DELETE VIETNAM PACKAGE: Delete successful');
        alert('Package deleted successfully!');
        fetchPackages(); // Refresh the list
      } else {
        const errorData = await response.json();
        console.log('DELETE VIETNAM PACKAGE: Delete failed:', errorData.error);
        console.log('DELETE VIETNAM PACKAGE: Full error response:', errorData);
